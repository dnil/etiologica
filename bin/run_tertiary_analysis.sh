...
#	    runme="awk '(\$7==\"PASS\") { print; }' < $patient_vcf > $patient_pass_vcf"
	...
	then
	    runme="$ANNOVARBIN/convert2annovar.pl -allallele -format vcf4 $patient_pass_vcf > $patient_avlist"
	...
	then
	    runme="$ANNOVAR_SUMMARIZE --buildver hg19 --ver1000g $KGENOMES_DB --verdbsnp $DB_SNP_VERSION_ANNOTATE --veresp $DB_ESP_VERSION --localdb $LOCAL_CLIN_DB --outfile $patient_annovar_summarize_base $patient_avlist $AVDBDIR"
	...
	patient_maf_filter=${patient_pass_avlist}.${KGENOMES_VERSION}_filtered
	if needsUpdate $patient_maf_filter $patient_pass_avlist $ANNOVARBIN/annotate_variation.pl
	then
	    runme="$ANNOVARBIN/annotate_variation.pl --filter --dbtype ${KGENOMES_DB}_all --score_threshold $ANNOVAR_1KG_MAF --buildver hg19 $patient_pass_avlist $AVDBDIR"
	    vanillaRun "$runme" "$patient_maf_filter" "result" "ANNOVAR --filter 1000g score $ANNOVAR_1KG_MAF"
	fi

	patient_local_clinical_filter_orig=${patient_maf_filter}.hg19_generic_filtered
	patient_local_clinical_filter=${patient_maf_filter}.hg19_local_clinical_filtered
	
	if [ "$LOCAL_DB_FILTER" == "yes" ]
	then
	    if needsUpdate $patient_local_clinical_filter $patient_maf_filter $ANNOVARBIN/annotate_variation.pl
	    then
		runme="$ANNOVARBIN/annotate_variation.pl --filter --dbtype generic --genericdbfile $LOCAL_CLIN_DB --buildver hg19 --score_threshold $LOCAL_MAF $patient_maf_filter $AVDBDIR"
		vanillaRun "$runme" "$patient_local_clinical_filter" "result" "ANNOVAR --filter local_clinical"
		mv $patient_local_clinical_filter_orig $patient_local_clinical_filter
	    fi

	    patient_danes_filter_orig=${patient_local_clinical_filter}.hg19_generic_filtered
	    patient_danes_filter=${patient_local_clinical_filter}.hg19_200danes_filtered
	    if needsUpdate $patient_danes_filter $patient_local_clinical_filter $ANNOVARBIN/annotate_variation.pl
	    then
		runme="$ANNOVARBIN/annotate_variation.pl --filter --dbtype generic --genericdbfile $LOCAL_DANES_DB --buildver hg19 --score_threshold $LOCAL_MAF $patient_local_clinical_filter $AVDBDIR"
	...
	# a file ignoring non-synonymous
	...
	# dbSNP filter also the rare file...
	patient_rare_dbSNP_filter=${patient_rare_filter}.hg19_${DB_SNP_VERSION}_filtered
	if needsUpdate $patient_rare_dbSNP_filter $patient_rare_filter $ANNOVARBIN/annotate_variation.pl
	then
	    runme="$ANNOVARBIN/annotate_variation.pl --filter --dbtype ${DB_SNP_VERSION} --buildver hg19 $patient_rare_filter $AVDBDIR"
	    vanillaRun "$runme" "$patient_rare_dbSNP_filter" "result" "ANNOVAR --filter db${DB_SNP_VERSION}"
	fi

	#  annotate the dbSNP filtered rare file
	patient_rare_dbSNP_exonic_variant=${patient_rare_dbSNP_filter}.exonic_variant_function
	if needsUpdate $patient_rare_dbSNP_exonic_variant $patient_rare_dbSNP_filter $ANNOVARBIN/annotate_variation.pl
	then
	    runme="$ANNOVARBIN/annotate_variation.pl --geneanno --splicing_threshold $ANNOVAR_SPLICING_THRESHOLD --buildver hg19 $patient_rare_dbSNP_filter $AVDBDIR"
	    vanillaRun "$runme" "$patient_rare_dbSNP_exonic_variant" "result" "ANNOVAR 1000g MAF $ANNOVAR_1KG_MAF 200danes localclinical db${DB_SNP_VERSION} --geneanno"
	fi
	
	# ignoring non-synonymous variants in an additional file
	patient_very_uncommon_not_syn=${patient_rare_dbSNP_exonic_variant}.not_syn
	if needsUpdate $patient_very_uncommon_not_syn $patient_rare_dbSNP_exonic_variant
	then
	    runme="cut -f2- $patient_rare_dbSNP_exonic_variant | awk '(\$1 != \"synonymous\") { print }' > $patient_very_uncommon_not_syn"
	    vanillaRun "$runme" "$patient_very_uncommon_not_syn" "result" "Ignore synonymous variants without any other severity prediction."
	fi

	# recessive looking for very uncommon (first with no PP2).

	patient_rare_dbSNP_variant_function=${patient_rare_dbSNP_filter}.variant_function
	patient_rare_dbSNP_splicing=${patient_rare_dbSNP_variant_function}.splicing
	if needsUpdate $patient_rare_dbSNP_splicing $patient_rare_dbSNP_variant_function
	then
	    runme="awk '(\$1 == \"exonic;splicing\" || \$1 == \"splicing\") { print }' $patient_rare_dbSNP_variant_function > $patient_rare_dbSNP_splicing"
	    vanillaRun "$runme" "$patient_rare_dbSNP_splicing" "result" "Extract splicing variants."
	fi

	# drop "dispensable" genes
	patient_very_uncommon_splicing_indisp=${patient_rare_dbSNP_splicing}.indisp
	if needsUpdate $patient_rare_dbSNP_splicing_indisp $patient_rare_dbSNP_splicing $ANNOVAR_DISPENSABLE
	then
	  runme="grep -v -w -f $ANNOVAR_DISPENSABLE $patient_rare_dbSNP_splicing > $patient_rare_dbSNP_splicing_indisp"
	  vanillaRun "$runme" "$patient_rare_dbSNP_splicing_indisp" "result" "Filter splicing variants in dispensable genes."
	fi

	# drop "dispensabe" genes
	patient_very_uncommon_not_syn_indisp=${patient_very_uncommon_not_syn}.indisp
	if needsUpdate $patient_very_uncommon_not_syn_indisp $patient_very_uncommon_not_syn $ANNOVAR_DISPENSABLE
	then
	  runme="grep -v -w -f $ANNOVAR_DISPENSABLE $patient_very_uncommon_not_syn > $patient_very_uncommon_not_syn_indisp"
	  vanillaRun "$runme" "$patient_very_uncommon_not_syn_indisp" "result" "Filter variants in dispensable genes."
	fi
	
	# hom | 2xhet. Evaluate splicing, but two different annotations for the same variant does not qualify for compound het.
	patient_recessive_variant_list=${patient_rare_dbSNP_variant_function}.indisp.recessive_model
	if needsUpdate $patient_recessive_variant_list $patient_very_uncommon_not_syn_indisp $patient_rare_dbSNP_splicing_indisp
	then
	   runme="cat ${patient_very_uncommon_not_syn_indisp} $patient_rare_dbSNP_splicing_indisp |perl -e 'while(<STDIN>) { chomp; @r=split(/\t+/); @transcripts=split(/[:;(]+/,\$r[1]); \$gene=\$transcripts[0]; \$gene=~s/\)//; \$gene_row=\$gene.\"\t\".join(\"\t\",@r).\"\n\"; \$mykey=\$r[2].\"_\".\$r[3].\"_\".\$r[4].\"_\".\$r[5].\"_\".\$r[6]; if ( exists(\$uniq{\$mykey}) && (\$uniq{mykey} == 1)) { } else { \$uniq{\$mykey} = 1; \$nvars{\$gene}++; } \$vars{\$gene}.=\$gene_row; if((\$r[7] eq \"hom\") || (\$r[7] eq \"het\" && \$nvars{\$gene} >1) ) { \$modelok{\$gene}=1; } } foreach \$gene (keys %modelok) {print \$vars{\$gene}};' > $patient_recessive_variant_list"
	   vanillaRun "$runme" "$patient_recessive_variant_list" "result" "Produce recessive model variant list"
	fi

	# Polyphen2 filter - the scary stuff
	...
	# annotate the PP2 filtered file
